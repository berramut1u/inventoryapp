...
export default function Moves() {
    ...
    useEffect(() => {
        // <ItemMove[]> tells TS we expect the nested payload
        api.get<ItemMove[]>('/inventory/moves')
                    ...
                    <div key={item.id} className="border p-4 rounded shadow mb-4">
                        ...
                        <ul className="list-disc list-inside text-sm">
                            {item.audits.length === 0
                                ...
                                : item.audits.map((a, i) => (
                                    <li key={i}>
                                        <strong>{item.name}</strong> was <em>{a.action}</em> by <strong>{a.performedBy}</strong><br />
                                        <small className="text-gray-500">
                                            {new Date(a.timestamp).toLocaleString() || 'Unknown time'}
                                        </small>
                                    </li>
                                ))
                            }
